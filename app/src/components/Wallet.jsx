...
import { decodeProposals, decodeMembers } from './buffer';
...
//     }
// }
export async function createProposal(client,contractIndex, description, amount,senderAddress) {
    const connectedToTestnet = await checkConnectedToTestnet(client);
    if (connectedToTestnet) {
        const txHash = await client.sendTransaction(
            senderAddress,
...
export async function renounceVotes(client,contractIndex, proposalID, votes,senderAddress) {
    const connectedToTestnet = await checkConnectedToTestnet(client);
    if (connectedToTestnet) {
        const txHash = await client.sendTransaction(
            senderAddress,
...
export async function insertFunds(client,contractIndex,amount,senderAddress){
    const connectedToTestnet = await checkConnectedToTestnet(client);
    if (connectedToTestnet) {
        const txHash = await client.sendTransaction(
            senderAddress,
...
export async function voteForProposal(client, contractIndex,proposalID, vote_decision,senderAddress){
    const connectedToTestnet = await checkConnectedToTestnet(client);
    if (connectedToTestnet) {
        const txHash = await client.sendTransaction(
            senderAddress,
...
export async function withdrawFunds(client,contractIndex,proposalID,senderAddress){
    const connectedToTestnet = await checkConnectedToTestnet(client);
    if (connectedToTestnet) {
        const txHash = await client.sendTransaction(
            senderAddress,
...
export async function getAllMembers(client, contractIndex) {
    const members = await client.getGrpcClient().invokeContract({
            contract: { index: BigInt(contractIndex), subindex: BigInt(0) },
...
export async function getAllProposals(client, contractIndex) {
    const proposals = await client.getGrpcClient().invokeContract({
        contract: { index: BigInt(contractIndex), subindex: BigInt(0) },
